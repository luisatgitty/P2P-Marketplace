...
import {
  Lock, Mail, Phone, Eye, EyeOff, CheckCircle2, AlertTriangle,
  Shield, KeyRound, User,
} from "lucide-react";
...
function Input({ type = "text", value, onChange, placeholder, disabled, rightEl }: {
  type?:        string;
  value:        string;
...
export default function SettingsPage() {
  ...
  const [phoneMsg,   setPhoneMsg]   = useState<{ text: string; type: "success"|"error" } | null>(null);
  ...
  function simulate(
    ...
    if (err) { msgSetter({ text: err, type: "error" }); return; }
    setter(true);
    setTimeout(() => {
      setter(false);
      onSuccess();
  ...
  const EyeBtn = ({ show, setShow }: { show: boolean; setShow: (v: boolean) => void }) => (
    <button type="button" onClick={() => setShow(!show)} className="text-stone-400 hover:text-stone-600 dark:hover:text-stone-300 transition-colors">
  ...
  return (
    <div className="p-5 sm:p-6 space-y-5 max-w-2xl">
      <div>
        <h2 className="text-xl font-extrabold text-stone-900 dark:text-stone-50">Settings</h2>
        <p className="text-sm text-stone-500 dark:text-stone-400 mt-0.5">Manage your admin account credentials and profile</p>
      </div>
      ...
      <div className="bg-[#1e2433] rounded-2xl p-5 flex items-center gap-4">
        <div className="w-14 h-14 rounded-full bg-gradient-to-br from-amber-500 to-amber-600 flex items-center justify-center text-white text-lg font-extrabold flex-shrink-0">
          {CURRENT_ADMIN.firstName.charAt(0)}{CURRENT_ADMIN.lastName.charAt(0)}
        </div>
        <div className="flex-1 min-w-0">
          <p className="text-base font-extrabold text-white">{CURRENT_ADMIN.firstName} {CURRENT_ADMIN.lastName}</p>
      ...
      {/* ── Profile ── */}
      <Section title="Profile Information" desc="Update your display name shown in the admin panel.">
        <div className="grid grid-cols-2 gap-4 mb-4">
          <div><FieldLabel>First Name</FieldLabel><Input value={firstName} onChange={setFirstName} /></div>
          <div><FieldLabel>Last Name</FieldLabel><Input value={lastName}  onChange={setLastName}  /></div>
        </div>
        {profileMsg && <div className="mb-4"><Toast msg={profileMsg.text} type={profileMsg.type} /></div>}
        <button
          type="button"
          disabled={savingProfile}
          onClick={() => simulate(setSavingProfile, setProfileMsg, () => {}, () => (!firstName.trim() || !lastName.trim()) ? "First and last name are required." : null)}
      ...
      {/* ── Password ── */}
      <Section title="Change Password" desc="Use a strong password with at least 8 characters, including uppercase, numbers, and symbols.">
        <div className="space-y-4 mb-4">
          <div>
            <FieldLabel>Current Password</FieldLabel>
            <Input type={showCurr ? "text" : "password"} value={currentPw} onChange={setCurrentPw} placeholder="Enter current password"
              rightEl={<EyeBtn show={showCurr} setShow={setShowCurr} />} />
          </div>
          <div>
            <FieldLabel>New Password</FieldLabel>
            <Input type={showNew ? "text" : "password"} value={newPw} onChange={setNewPw} placeholder="Enter new password (min 8 chars)"
              rightEl={<EyeBtn show={showNew} setShow={setShowNew} />} />
          </div>
          <div>
            <FieldLabel>Confirm New Password</FieldLabel>
            <Input type={showConf ? "text" : "password"} value={confirmPw} onChange={setConfirmPw} placeholder="Re-enter new password"
          ...
          <div className="mb-4 space-y-1.5">
            {[
              ...
              <div key={label} className="flex items-center gap-2">
                <CheckCircle2 className={cn("w-3.5 h-3.5 flex-shrink-0", ok ? "text-teal-500" : "text-stone-300 dark:text-stone-600")} />
                <span className={cn("text-xs", ok ? "text-stone-700 dark:text-stone-200" : "text-stone-400 dark:text-stone-500")}>{label}</span>
              </div>
            ))}
          </div>
        )}

        {pwMsg && <div className="mb-4"><Toast msg={pwMsg.text} type={pwMsg.type} /></div>}
        <button
          type="button"
          disabled={savingPw}
          onClick={() => simulate(setSavingPw, setPwMsg,
      ...
      {/* ── Email ── */}
      <Section title="Change Email Address" desc="A verification email will be sent to your new address before the change takes effect.">
        <div className="space-y-4 mb-4">
          <div>
          ...
          <div>
            ...
            <Input type="email" value={newEmail} onChange={setNewEmail} placeholder="Enter new email address" />
          </div>
          <div>
            <FieldLabel>Current Password (to confirm)</FieldLabel>
            <Input type="password" value={emailPw} onChange={setEmailPw} placeholder="Enter your password to confirm" />
          </div>
        </div>
        {emailMsg && <div className="mb-4"><Toast msg={emailMsg.text} type={emailMsg.type} /></div>}
        <button
          type="button"
          disabled={savingEmail}
          onClick={() => simulate(setSavingEmail, setEmailMsg,
      ...
      {/* ── Phone ── */}
      <Section title="Change Mobile Number" desc="Update the mobile number associated with your admin account.">
        <div className="space-y-4 mb-4">
          <div>
          ...
          <div>
            ...
            <div className="flex gap-2">
              <div className="flex items-center justify-center bg-stone-100 dark:bg-[#13151f] border border-stone-200 dark:border-[#2a2d3e] rounded-xl px-3 text-sm text-stone-500 dark:text-stone-400 font-medium w-14 flex-shrink-0">+63</div>
              <Input value={newPhone} onChange={setNewPhone} placeholder="9XX XXX XXXX" />
            </div>
          </div>
          <div>
            <FieldLabel>Current Password (to confirm)</FieldLabel>
            <Input type="password" value={phonePw} onChange={setPhonePw} placeholder="Enter your password to confirm" />
          </div>
        </div>
        {phoneMsg && <div className="mb-4"><Toast msg={phoneMsg.text} type={phoneMsg.type} /></div>}
        <button
          type="button"
          disabled={savingPhone}
          onClick={() => simulate(setSavingPhone, setPhoneMsg,
      ...
      <div className="bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-800 rounded-2xl p-4">
        <div className="flex items-center gap-2 mb-2">
          ...
          <p className="text-sm font-bold text-amber-800 dark:text-amber-300">Security Reminders</p>
        </div>
