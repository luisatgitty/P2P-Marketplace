...
const ALL_LISTINGS: PostCardProps[] = [
...
export default function Home() {
  const [activeTab, setActiveTab] = useState<"all" | "sale" | "rent" | "service">("all");
  ...
  return (
    <>
      ...
      <CategoryFilter totalCount={filteredListings.length} onTabChange={setActiveTab} />
      ...
      <main id="listings" className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 fade-in delay-2">
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3 sm:gap-4 pt-28">
          {filteredListings.map((listing) => <PostCard key={listing.id} {...listing} />)}
        </div>
