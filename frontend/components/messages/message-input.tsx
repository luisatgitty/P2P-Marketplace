...
export default function MessageInput({
  ...
  const handleSend = async () => {
    ...
    if ((!trimmed && stagedMedia.length === 0) || disabled || preparingSend)
      return;
  ...
  const canSend =
    (value.trim().length > 0 || stagedMedia.length > 0) &&
    !disabled &&
    !preparingSend;
        ...
        <div className='flex items-end'>
          ...
          <div className='flex-1 min-w-0 mx-2'>
            <Textarea
              ...
              placeholder={
                replyTo ? `Reply to ${replyTo.senderName}…` : 'Type a message…'
              }
              disabled={disabled}
              rows={1}
          ...
          >
            <SendHorizontal
              size={15}
              className={canSend ? '-translate-x-px' : ''}
            />
          </Button>
        </div>
      </div>
