...
export default function CategoryFilter({ onTabChange, onCategoryChange, onSortChange, totalCount = 0 }: CategoryFilterProps) {
  ...
  return (
    <div className="bg-stone-50 dark:bg-[#161d2b] border-b border-stone-200 dark:border-white/10 sticky top-26.25 z-40 shadow-sm">
      <div className="max-w-7xl mx-auto px-6 sm:px-8 lg:px-10">
        <div className="flex gap-8 border-b border-stone-100 dark:border-white/10 overflow-x-auto no-scroll">
          {TABS.map((tab) => (
            <button key={tab.value}
              ...
              className={`text-base font-medium py-4 whitespace-nowrap border-b-2 transition-colors ${
                activeTab === tab.value
                  ? "border-stone-800 dark:border-white text-stone-800 dark:text-white"
              ...
              <button key={cat.value}
                ...
                className={`shrink-0 border rounded-full px-5 py-2 text-sm font-medium whitespace-nowrap transition-all ${
                  activeCategory === cat.value
                    ? "bg-stone-800 dark:bg-white dark:text-stone-900 text-stone-100 border-stone-800 dark:border-white"
                    : "border-stone-200 dark:border-white/20 text-stone-600 dark:text-stone-300 hover:bg-stone-800 hover:text-stone-100 dark:hover:bg-white dark:hover:text-stone-900"
                }`}>
                {cat.emoji} {cat.label}
              </button>
            ))}
          ...
          <div className="flex items-center gap-4 shrink-0">
            {totalCount > 0 && (
              <p className="text-base text-stone-500 dark:text-stone-400 hidden sm:block">
                <span className="font-semibold text-stone-700 dark:text-white">{totalCount.toLocaleString()}</span> listings
              </p>
            )}
            <select value={activeSort}
              onChange={(e) => { setActiveSort(e.target.value); onSortChange?.(e.target.value); }}
              className="bg-transparent dark:bg-[#161d2b] dark:text-white border border-stone-200 dark:border-white/20 rounded-full text-sm text-stone-600 px-4 py-2 cursor-pointer outline-none hover:border-stone-400 transition-colors appearance-none">
