...
export function getCitiesByProvince(provinceCode: string): Promise<LocationOption[]> {
  return fetchLocations(`/locations/cities?provinceCode=${encodeURIComponent(provinceCode)}`);
}

export function getBarangaysByCity(cityCode: string): Promise<LocationOption[]> {
  return fetchLocations(`/locations/barangays?cityCode=${encodeURIComponent(cityCode)}`);
}
